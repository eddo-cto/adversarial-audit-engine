BEGIN{FS="\t"; m["Jan"]=1;m["Feb"]=2;m["Mar"]=3;m["Apr"]=4;m["May"]=5;m["Jun"]=6;m["Jul"]=7;m["Aug"]=8;m["Sep"]=9;m["Oct"]=10;m["Nov"]=11;m["Dec"]=12}
function dk(s,  a,n){ if(s=="-"||s=="")return 0; n=split(s,a," "); if(n<3)return 0; if(!(a[1] in m))return 0; sub(/,/,"",a[2]); return a[3]*10000+m[a[1]]*100+a[2] }
function bk(s){ if(s=="Uncertain significance")return "V"; if(s=="Pathogenic"||s=="Likely pathogenic"||s=="Benign"||s=="Likely benign"||s=="Pathogenic/Likely pathogenic"||s=="Benign/Likely benign")return "D"; return "O" }
function nbb(n){ return n<=4?"3-4":(n<=9?"5-9":"10+") }
function proc(  i,minv,startb,i1,i2,i3,d1,d2,d3,cD,cV,fin){
 if(cnt<3)return
 if(hasx)return                 # solo PEER-ONLY (isola l'accumulo dall'autorita')
 minv=1e18; for(i=1;i<=cnt;i++){if(KD[i]<minv){minv=KD[i];startb=KB[i]}}
 if(startb!="V")return
 i1=0;d1=-1;i2=0;d2=-1;i3=0;d3=-1
 for(i=1;i<=cnt;i++){ if(KD[i]>d1){d3=d2;i3=i2;d2=d1;i2=i1;d1=KD[i];i1=i} else if(KD[i]>d2){d3=d2;i3=i2;d2=KD[i];i2=i} else if(KD[i]>d3){d3=KD[i];i3=i} }
 cD=0;cV=0
 if(KB[i1]=="D")cD++;else if(KB[i1]=="V")cV++
 if(KB[i2]=="D")cD++;else if(KB[i2]=="V")cV++
 if(KB[i3]=="D")cD++;else if(KB[i3]=="V")cV++
 if(cD>=2)fin="D";else if(cV>=2)fin="V";else fin=KB[i1]
 kk++; FY[kk]=int(minv/10000); NB[kk]=nbb(nsub); RSV[kk]=(fin=="D")?1:0
}
{ vid=$1
  if(vid!=cur){ if(cur!="")proc(); cur=vid; cnt=0; hasx=0; nsub=0; delete SB }
  d=dk($3); if(d>0){cnt++; KD[cnt]=d; KB[cnt]=bk($2); if(d>gmax)gmax=d}
  if($4=="reviewed by expert panel"||$4=="practice guideline")hasx=1
  if(!($5 in SB)){SB[$5]=1;nsub++}
}
END{ if(cur!="")proc()
 gy=int(gmax/10000)
 for(i=1;i<=kk;i++){ age=gy-FY[i]
   ab=(age<=2?"0-2":(age<=5?"3-5":(age<=9?"6-9":"10+")))
   key=ab"|"NB[i]; TOT[key]++; if(RSV[i])RES[key]++
   rowtot[ab]++; if(RSV[i])rowres[ab]++ }
 print "Snapshot year:",gy,"| variabili peer-only born-VUS classificate:",kk
 print ""
 print "TASSO DI RISOLUZIONE (%) — righe: eta'-nel-sistema (anni) ; colonne: n. sottomettitori"
 printf "%-8s | %-14s | %-14s | %-14s | %-14s\n","eta'","3-4","5-9","10+","(riga tutti)"
 split("0-2 3-5 6-9 10+",AR," ")
 split("3-4 5-9 10+",NBINS," ")
 for(a=1;a<=4;a++){ ab=AR[a]; line=sprintf("%-8s |",ab)
   for(c=1;c<=3;c++){ nb=NBINS[c]; k=ab"|"nb
     if(TOT[k]>0) line=line sprintf(" %5.1f%% (n=%6d) |",100*RES[k]/TOT[k],TOT[k])
     else line=line sprintf(" %5s  (n=%6d) |","--",0) }
   line=line sprintf(" %5.1f%% (n=%6d)",(rowtot[ab]?100*rowres[ab]/rowtot[ab]:0),rowtot[ab])
   print line }
}
