...
4. **Deep-causal** — run it on any **HIGH-posta or richly-structured** artifact: root-cause clustering, forward/backward chiasm, gated scenarios. The only reason to skip is a small, low-stakes artifact (the Freno — do not over-engineer a trivial one). Fold its roots into your findings and, in the payload's `execution`, mark `deep_causal` as `ran`.
