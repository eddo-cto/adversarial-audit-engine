...
5. **Deterministic core.** Do this by the book — do NOT reverse-engineer `aae` by hand (never import its modules to list enums or guess class names; a real run wasted a turn guessing a non-existent `TaxonomyCell`).
   1. Get the exact contract in one call: `python3 "$CLAUDE_PLUGIN_ROOT/scripts/run_core.py" --schema` (on Windows use `python` if `python3` is absent). It prints the payload template + the live enum vocabulary (taxonomy_cell, defect_class, posta, evidence_base, cost_to_fix, action_state) and the rules.
   2. Write your collected findings to `findings.json` **matching that schema** — include `source_text` (the artifact's verbatim text, to arm the grounding gate) and, on a HIGH-posta run, at least one `action_state: deliberately_discarded` hypothesis.
   3. Run `python3 "$CLAUDE_PLUGIN_ROOT/scripts/run_core.py" findings.json` → validated ledger, verdicts via the state machine, dedup, metrics, manifest. No ARTIFACT_DEFECTIVE verdict without a recorded defense; verdicts are assigned by the code, never by you.
6. **Independence**: build the eye with `aae.external_eye_from_env()` (from `AAE_EYE`). If it returns a client, **actually call it** for an independent re-read, then export `AAE_EXTERNAL_ATTESTED_IDENTITY=<eye.identity>` so `run_core.py` credits **level 3** — attested because the adapter really ran, never merely configured. If it returns `None`, record the run as level 1 (do not fake an external identity).
